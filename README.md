...
* 创建一个application
    ...
    @SpringBootApplication:相当于@Configuration, @EnableAutoConfiguration 和 @ComponentScan 具体见spring开发
    
    @EnableConfigServer：比较关键，具体见源代码注释

* 运行config-server,获取git上的资源信息遵循如下规则
        ...
        spring:
          ...
          cloud:
            config:
              server:
                git:
                  uri: http://code.goodrain.com/demo/springcloud-config.git
                  searchPaths: config
...
* 创建一个restcontroller
    ...
    @RefreshScope：Spring Bean都会被重新初始化并载入新的配置内容
